...
if [[ ${EBUILD_PHASE} == "setup" ]]; then
	...
	ehooks_apply() {
		...
		local \
			bugapnd="eerror \"S: '\${S}'\"" \
			bugmsg="eerror \"${color_bold}Please submit ehooks bug at ${color_norm}'https://github.com/c4pp4/gentoo-lomiri/issues'.\"" \
		...
		if [[ ${EHOOKS_ACCEPT} != "yes" ]]; then
			...
			eerror "Warning!"
			eerror
			eerror "Patches from gentoo-lomiri overlay will be applied to ${CATEGORY}/${PN} and some other packages from the Gentoo tree via ehooks patching system."
		...
		echo "${color_red}${color_bold}>>> Loading gentoo-lomiri ehooks ...${color_norm}"
