...
DESCRIPTION="Setup the Lomiri build environment and ehooks patching system"
HOMEPAGE="https://github.com/renegart/gentoo-unity7"
SRC_URI=""
...
pkg_setup() {
	...
	[[ -f ${EROOT}/etc/gentoo-lomiri/timestamps ]] && cp "${EROOT}"/etc/gentoo-lomiri/timestamps "${S}"/timestamps.old
}
...
src_install() {
	...
	ebegin "Generating ${EROOT}/etc/gentoo-limiri/timestamps file"
	for x in ${IUSE_EHOOKS}; do
		x="${x#+}"

		## Find out if there is USE flag change.
		use "${x}" && pkg_flag=1 || pkg_flag=0
		"${PORTAGE_QUERY_TOOL}" has_version / lomiri-base/gentoo-lomiri-env["${x}"] && sys_flag=1 || sys_flag=0
	...
	dosym -r "${REPO_ROOT}"/version_control.sh /usr/bin/gentoo-lomiri-ver
...
pkg_postinst() {
	...
	local \
		...
		prev_shopt=$(shopt -p nullglob) \
		x=$("${PORTAGE_QUERY_TOOL}" get_repo_path / gentoo-lomiri)/version_control.sh

	shopt -s nullglob
	local -a cfg_files=( "${EROOT}"/etc/gentoo-lomiri/._cfg*timestamps )
	${prev_shopt}
